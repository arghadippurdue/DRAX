...
def perform_sensitivity_analysis(model, net_params, sparsities, test_func, group):
    ...
    for param_name in net_params:
        ...
        for sparsity_level in sparsities:
            ...
            print("\nTesting sensitivity of %s [%0.1f%% sparsity]" % (param_name, sparsity_level*100))  # AD
...
def sensitivities_to_png(sensitivities, fname):
    ...
    for param_name, sensitivity in sensitivities.items():
        sense = [values[0] for sparsity, values in sensitivity.items()]  # AD
        # sense = [values[1] for sparsity, values in sensitivity.items()]
        sparsities = [sparsity for sparsity, values in sensitivity.items()]
        plt.plot(sparsities, sense, label=param_name, marker="x")

    plt.ylabel('Accuracy')  # AD
    # plt.ylabel('top5')
    plt.xlabel('sparsity')
...
def sensitivities_to_csv(sensitivities, fname):
    ...
    with open(fname, 'w') as csv_file:
        ...
        # write the header
        # writer.writerow(['parameter', 'sparsity', 'overall acc', 'mean_class_acc', 'loss'])  # AD
        writer.writerow(['parameter', 'sparsity', 'top1', 'top5', 'loss'])    # Keep it same for easier analysis
        for param_name, sensitivity in sensitivities.items():
