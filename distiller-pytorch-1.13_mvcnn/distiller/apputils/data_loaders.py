...
DATASETS_NAMES = ['imagenet', 'cifar10', 'mnist', 'modelnet'] #AD
...
def classification_get_input_shape(dataset):
    if dataset == 'imagenet':
        ...
        return 12, 3, 224, 224    # <FIXME> hardcoded 'num_views'
    else:
...
def __dataset_factory(dataset, arch):
    return {'cifar10': cifar10_get_datasets,
            ...
            'modelnet': partial(modelnet_get_datasets)}.get(dataset, None)
...
#AD
def modelnet_get_datasets(data_dir, load_train=True, load_test=True):
    """
    ...
    train_dataset = None
    if load_train:
        print('---- Loading ModelNet40 training set ----', train_dir)
        train_dataset = MultiviewImgDataset(train_dir, test_mode=False)
        print('num_train_files: '+str(len(train_dataset.filepaths)))

    test_dataset = None
    if load_test:
        print('---- Loading ModelNet40 testing set ----', test_dir)
        test_dataset = MultiviewImgDataset(test_dir, test_mode=True)
        print('num_val_files: '+str(len(test_dataset.filepaths)))
        print(test_dataset[0][1].unsqueeze(0).size())
...
def __image_size(dataset):
    ...
    try:
      return dataset[0][0].unsqueeze(0).size()      # for datasets other than 'modelnet'
    except:
      return dataset[0][1].unsqueeze(0).size()      # Specially for 'modelnet'
...
class MultiviewImgDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, scale_aug=False, rot_aug=False, test_mode=False, \
        ...
        # If "test_mode=True"
        if self.test_mode:
            # Just convert the image to tensor and then normalize
